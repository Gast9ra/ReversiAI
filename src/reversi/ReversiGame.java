...
public class ReversiGame extends Game {
    ...
    public boolean isValidCell(int row, int col, int role) { //проверка на видность координат
    ...
    int approximateUtility(int r) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                if (board[i][j] == r) {
                    ownScore++;
                    if (isCorner(i, j)) { //угол
                        ownScore += 35;
                    } else if (isSide(i, j)) { //сторона
                        ownScore += 20;
                    } else if (isXsquare(i, j)) { //серединная клетка
                        ownScore -= 35;
                    } else if (isCsquare(i, j)) { //клетка перед углом
                        ownScore -= 20;
                    }
                }
            }
